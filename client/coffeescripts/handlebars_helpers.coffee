...
# import enrollmentName from './util/enrollmentName'
import _Handlebars from 'handlebars/runtime'
import I18nObj from 'i18nObj' #  'i18nObj' gets the extended I18n object with all the extra functions (interpolate, strftime, ...)
# import $ from 'jquery'
# import _ from 'underscore'
import htmlEscape from 'str/htmlEscape'
# import semanticDateRange from './util/semanticDateRange'
# import dateSelect from './util/dateSelect'
# import mimeClass from './util/mimeClass'
...
Handlebars.registerHelper name, fn for name, fn of {
  t : (args..., options) ->
    # disabling for now, simply render what's in the body
  ...
  dateString : (datetime) ->
    return '' unless datetime
    I18nObj.l "date.formats.medium", datetime
  ...
  tDateToString : (date = '', i18n_format) ->
    ...
    I18nObj.l "date.formats.#{i18n_format}", fudged
  ...
  tTimeToString : (date = '', i18n_format) ->
    ...
    date = tz.parse(date) unless _.isDate date
    fudged = $.fudgeDateForProfileTimezone(tz.parse(date))
    I18nObj.l "time.formats.#{i18n_format}", fudged

  tTimeHours : (date = '') ->
    if date.getMinutes() == 0 and date.getSeconds() == 0
      I18nObj.l "time.formats.tiny_on_the_hour", date
    else
      I18nObj.l "time.formats.tiny", date
  ...
  uniqid: (context) ->
    context = (this || window) if arguments.length <= 1
  ...
  n:(number, {hash: {precision, percentage, strip_insignificant_zeros}}) ->
    I18nObj.n(number, {precision, percentage, strip_insignificant_zeros})
